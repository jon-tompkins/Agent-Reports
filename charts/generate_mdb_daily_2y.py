#!/usr/bin/env python3
"""Generate MDB daily 2-year price chart for deep dive report"""
...
ax1.plot(df.index, df['Close'], color='#ff6b6b', linewidth=2, label='MDB Price')
ax1.plot(df.index, df['Close'].rolling(50).mean(), color='#f4a261', linewidth=2, label='50-day MA', alpha=0.9)
ax1.plot(df.index, df['Close'].rolling(100).mean(), color='#2a9d8f', linewidth=2, label='100-day MA', alpha=0.9)
ax1.plot(df.index, df['Close'].rolling(200).mean(), color='#264653', linewidth=2, label='200-day MA', alpha=0.9)
...
ax1.set_title('MDB (MongoDB) - Daily Chart (2 Years)', fontsize=18, fontweight='bold', pad=20)
...
plt.tight_layout()
plt.savefig('/home/ubuntu/clawd/Agent-Reports/charts/MDB-daily-2y.png', 
            dpi=150, bbox_inches='tight', facecolor='#1a1a2e', edgecolor='none')
print(f"Daily 2Y chart saved to /home/ubuntu/clawd/Agent-Reports/charts/MDB-daily-2y.png")