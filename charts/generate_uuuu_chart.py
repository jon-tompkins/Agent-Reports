#!/usr/bin/env python3
"""Generate UUUU price chart for deep dive report"""
...
# Fetch UUUU data - 1 year
ticker = yf.Ticker("UUUU")
...
ax1.set_title('UUUU (Energy Fuels Inc.) - 1 Year Price Chart', fontsize=16, fontweight='bold', pad=20)
...
plt.tight_layout()
plt.savefig(f'/home/ubuntu/clawd/Agent-Reports/charts/uuuu-price-{datetime.now().strftime("%Y-%m-%d")}.png', 
            dpi=150, bbox_inches='tight', facecolor='#1a1a2e', edgecolor='none')
print(f"Chart saved to /home/ubuntu/clawd/Agent-Reports/charts/uuuu-price-{datetime.now().strftime('%Y-%m-%d')}.png")